...
# 后台IP设置
export Ipv4_ipaddr="192.168.3.10"            # 修改openwrt后台地址(填0为关闭)
export Netmask_netm="255.255.255.0"         # IPv4 子网掩码（默认：255.255.255.0）(填0为不作修改)
export Op_name="EdisonChan"                # 修改主机名称为OpenWrt-123(填0为不作修改)
...
# 旁路由选项
export Gateway_Settings="192.168.3.1"                 # 旁路由设置 IPv4 网关(填入您的网关IP为启用)(填0为不作修改)
...
export Customized_Information="EdisonChan ✈️ $(TZ=UTC-8 date "+%Y.%m.%d")"  # 个性签名,你想写啥就写啥，(填0为不作修改)
...
sed -i 's/"实时流量监测"/"流量监控"/g' `egrep "实时流量监测" -rl ./`
sed -i 's/"KMS 服务器"/"KMS服务器"/g' `egrep "KMS 服务器" -rl ./`
